import { listInterfaceInfoVOByPageUsingPOST } from '@/services/api-tomato-backend  /interfaceInfoController';
import { addUserInterfaceInfoUsingPOST } from '@/services/api-tomato-backend  /userInterfaceInfoController';
...
const Index: React.FC = () => {
  ...
  const columns: ProColumns<API.InterfaceInfoVO>[] = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return record.status === 0 ? (
        ...
        ) : record.isOpenInterface ? (
          <Button
            key="onlineUse"
            type="primary"
            style={{ backgroundColor: 'skyblue', width: '88px' }}
          >
            已开通
          </Button>
        ) : (
          <Button
            ...
            onClick={async () => {
              ...
              if (res.code === 0) {
                message.success('申请成功!请前往 ‘我的接口’ 调用!');
                // 刷新表格
                await loadData();
              }
            }}
  ...
  return (
    <PageContainer>
      <Layout>
        <div style={{ marginLeft: '21%', marginBottom: '-4%' }}>
